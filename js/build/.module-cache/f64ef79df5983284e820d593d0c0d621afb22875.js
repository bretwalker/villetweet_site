 /** @jsx React.DOM */
    ...
    var TweetList = React.createClass({displayName: 'TweetList',
        getInitialState: function() {
            return { tweets: this.props.tweets, source: null };
        },
        updateUserData: function(e) {
            var _this = this;
            update = JSON.parse(e.data)

		    this.props.tweets.shift();
		    this.props.tweets.push();
		    
		    this.setState({'users': this.props.tweets});
        },
        componentDidMount: function() {
            this.state.source = new EventSource('http://localhost:8080/recent-tweets');
			this.state.source.onmessage = this.updateTweets;
        },
        render: function() {
            var _this = this;
            this.props.tweets.sort(sortScoreDescNameAsc).forEach(function(user) {
                if(lastScore != user.score) {
                    rank++;
                }
                rows.push(ScoredTwitterThing({name: user.name, score: user.score, rank: rank, key: user.name, type: _this.props.type}));
    ...
    var App = React.createClass({displayName: 'App',
        ...
        render: function() {
            ...
            } else if(this.state.currentPage == 'recnet') {
        ...
        changeNav: function(page, days) {
            ...
            $.ajax({
              ...
              url: 'http://localhost:8081/' + page + '?days=' + days,
                ...
                if(this.state.currentPage == 'recent') {
