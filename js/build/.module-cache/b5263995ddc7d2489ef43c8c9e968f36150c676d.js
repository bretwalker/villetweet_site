 /** @jsx React.DOM */
    ...
    var baseUrl = 'http://louiewatch.com'
    ...
    var ScoredTwitterThingList = React.createClass({displayName: 'ScoredTwitterThingList',
        ...
        componentDidMount: function() {
            this.state.source = new EventSource(baseUrl + ':8080/subscribe/eps/' + this.props.type);
			this.state.source.onmessage = this.updateUserData;
        },
        componentWillUpdate: function(nextProps, nextState) {
            if(nextProps.type != this.props.type) {
                this.state.source.close();
                this.state.source = new EventSource(baseUrl + ':8080/subscribe/eps/' + nextProps.type);
                this.state.source.onmessage = this.updateUserData;
            }
        },
    ...
    var TweetList = React.createClass({displayName: 'TweetList',
        ...
        componentDidMount: function() {
            this.state.source = new EventSource(baseUrl + ':8080/subscribe/recent-tweets');
    ...
    var TweetCount = React.createClass({displayName: 'TweetCount',
        ...
        componentDidMount: function() {
            ...
            $.ajax({
              type: 'GET',
              url: baseUrl + ':8081/tweet-count',
            }).done(function(data) {
                _this.props.tweetCount += data.tweet_count;
            });
            
            this.state.source = new EventSource(baseUrl + ':8080/subscribe/eps/tweet-count');
			this.state.source.onmessage = updateTweetCount;
        },
        render: function() {
            return (               
                React.DOM.div({id: "tweet-count"}, 
                    this.props.tweetCount, " tweets and counting..."
                )
            );
        }
    });
    ...
    var App = React.createClass({displayName: 'App',
        ...
        changeNav: function(page, days) {
            ...
            $.ajax({
              type: "GET",
              url: baseUrl + ':8081/' + page + '?days=' + days,
            }).done(function(data) {
                thingPositions = {};
    ...
    $(document).ready(function() {
        $.ajax({
          type: 'GET',
          url: baseUrl + ':8081/prolific?days=0',
        }).done(function(data) {
            if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
                data.length = 100;
            }
